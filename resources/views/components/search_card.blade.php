<div class="product-card" data-id="{{ $id }}" data-cpu="{{ $cpu_formatted }}" data-gpu="{{ $gpu_formatted }}" data-room="{{ $room }}" data-floor="{{ $floor }}" data-ram="{{ $ram }}" data-token="{{ $price }}">
    <button type="button" onclick="openProductPage({{ $id }})">
        <div class="rounded-xl shadow-2xl w-64 transform transition-transform hover:scale-105" id="card">
        <img src="{{ $image1 ?? '../img/ad/placeholder1.png' }}" alt="" class="object-cover rounded-t-xl aspect-square h-75 w-auto">
            <div class="bg-lime-800 text-white flex py-2 px-2 justify-center">
                <p class="font-bold">{{ $name ?? 'Lorem Ipsum' }}</p>
            </div>
            <div class="p-4 pb-8">
                <div class="flex flex-wrap gap-y-2">
                    @if ($cpu_formatted == 'intel')
                        <span class="bg-blue-100 text-blue-800 text-xs font-semibold me-2 px-2.5 py-0.5 rounded-sm">
                            Intel
                        </span>
                    @elseif ($cpu_formatted == 'amd')
                ...
                <div class="transition-all duration-300 ease-in-out group-hover:opacity-0">
                    <div class="bg-gray-50 text-lime-700 border-t border-lime-700 flex py-2 px-2 justify-center rounded-b-xl gap-2">
                        ...
                        <p class="font-bold">{{ $price ?? '99' }} Token / Jam</p>
                    </div>
                </div>
