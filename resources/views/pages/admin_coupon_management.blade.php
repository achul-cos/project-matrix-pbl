...
@section('title', 'Admin - Management Coupon')

@section('content')
<div class="flex-1 px-8 py-10">
  <section id="title">
    <h1 class="text-3xl font-bold mb-6 text-slate-900">Manajemen Kupon</h1>
  </section>
    ...
    <div class="p-4 bg-white border-2 border-sky-300 shadow-lg rounded-2xl min-w-[14rem]">
      <div data-modal-target="add-coupon-modal" data-modal-toggle="add-coupon-modal" class="transform transition-transform hover:scale-105 justify-items-center active:scale-95 group -mt-2 cursor-pointer">
        ...
        <div class="-mt-10 bg-sky-500 p-4 rounded-lg z-0">
          ...
          <div class="font-light text-sm text-white text-center max-w-36 mt-4">Tambah Kupon Baru</div>
        </div>
      </div>
    </div>
  </section>
      ...
      <tbody>
        @foreach ($coupons as $coupon)
          <tr class="bg-sky-50 rounded-xl">
            ...
            <td class="p-3">
              <form action="{{ route('admin.coupon.destroy', $coupon->id) }}" method="POST" onsubmit="return confirm('Yakin hapus kupon ini?')">
                @csrf
                @method('DELETE')
  ...
  <div class="relative w-full max-w-4xl mx-auto p-6">
    <div class="bg-white rounded-lg shadow-xl">
      <div class="flex items-center justify-between p-4 border-b bg-sky-700 rounded-t">
        <h3 class="text-lg font-semibold text-white">Tambah Kupon Baru</h3>
      ...
      <div class="p-6">
        <form action="{{ route('admin.coupon.store') }}" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-4">
          @csrf
          <input type="text" name="name" placeholder="Nama Kupon" required class="w-full border px-4 py-2 rounded-lg">
          <input type="text" name="code" placeholder="Kode Kupon" required class="w-full border px-4 py-2 rounded-lg">
          ...
          <div class="md:col-span-2 flex justify-end">
            <button type="submit" class="bg-sky-600 hover:bg-sky-700 text-white font-bold py-2 px-6 rounded-lg">Tambah Kupon</button>
          </div>
        </form>
      </div>
    </div>
