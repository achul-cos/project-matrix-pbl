    ...
    <section class="flex-1 px-8 py-10">
        <div class="bg-white p-6 rounded-2xl border-4 border-[#565885] shadow-xl">
            ...
            <div class="bg-white rounded-lg shadow-md overflow-hidden">
                ...
                <!-- Baris Data - Generated with PHP/JS loop in real app -->
                @for ($i = 1; $i <= 10; $i++)
                <div class="grid grid-cols-4 p-4 {{ $i < 10 ? 'border-b' : '' }}">
                    @foreach (['A', 'B', 'C', 'D'] as $floor)
                    <div class="flex items-center">
                        <input type="checkbox" id="{{ $floor.$i }}" class="h-5 w-5 
                            {{ $floor === 'A' ? 'text-blue-600' : '' }}
                            {{ $floor === 'B' ? 'text-red-600' : '' }}
                            {{ $floor === 'C' ? 'text-yellow-600' : '' }}
                            {{ $floor === 'D' ? 'text-green-600' : '' }} mr-2">
                        <label for="{{ $floor.$i }}" class="text-gray-800">{{ $floor.$i }}</label>
                    </div>
                    @endforeach
                </div>
                @endfor
