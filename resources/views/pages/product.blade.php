  ...
  <section class="p-8 px-32 mt-6 max-sm:px-4 max-md:px-16 bg-white rounded-2xl grid md:grid-cols-2 gap-10">
    ...
    <div class="flex flex-col justify-between">
      ...
      <div class="py-10">
          ...
          <div id="rental-schedule-container">
              @if($rentals->count() > 0)
                  ...
                  <div class="mt-6 flex justify-between items-center">
                      ...
                      <div class="flex space-x-2">
                          ...
                          @else
                              <button onclick="loadRentalsPage({{ $rentals->currentPage() - 1 }})" class="px-4 py-2 bg-[#556B2F] text-white rounded-lg hover:bg-[#6e8239] transition">
                                  <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                      <path fill-rule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd" />
                                  </svg>
                              </button>
...
<script>
function loadRentalsPage(page) {
    // Ubah URL untuk match dengan route yang ada
    fetch(`/product/{{ $product->id }}?page=${page}`, {
        headers: {
    ...
    .then(data => {
        ...
        // Jika tidak ada data rentals
        if (rentals.length === 0) {
            document.getElementById('rental-schedule-container').innerHTML = `
                <div class="text-center py-10 bg-white rounded-2xl shadow-lg">
                    <div class="inline-block p-6 bg-[#f8f9e8] rounded-full mb-4">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-16 w-16 text-[#556B2F]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
        ...
        rentals.forEach(rental => {
            const startDate = new Date(rental.booked_start);
            const endDate = new Date(rental.booked_end);
            
            tableHTML += `
                <tr class="border-b border-gray-200 hover:bg-[#f8f9e8] transition duration-300">
                    <td class="py-4 px-6">
                        <div class="flex items-center">
                            <span class="font-mono font-semibold">${rental.user.name}</span>
                    ...
                    <td class="py-4 px-6">
                        ...
                        <div class="text-sm text-gray-500">${formatTime(startDate)}</div>
                    </td>
                    <td class="py-4 px-6">
                        <div class="font-medium">${formatDate(endDate)}</div>
                        <div class="text-sm text-gray-500">${formatTime(endDate)}</div>
                    </td>
                    <td class="py-4 px-6">
                        <div class="flex items-center">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-[#556B2F] mr-2" viewBox="0 0 20 20" fill="currentColor">
        ...
        let paginationHTML = `
            <div class="mt-6 flex justify-between items-center">
                <div class="text-sm text-gray-600">
                    Menampilkan ${pagination.from || 0} - ${pagination.to || 0} dari ${pagination.total || 0} penyewaan
                </div>
                <div class="flex space-x-2">
        `;
        ...
        if (pagination.current_page > 1) {
            paginationHTML += `
                <button onclick="loadRentalsPage(${pagination.current_page - 1})" class="px-4 py-2 bg-[#556B2F] text-white rounded-lg hover:bg-[#6e8239] transition">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                        <path fill-rule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd" />
                    </svg>
                </button>
        ...
        // Gabungkan semua
        const fullHTML = tableHTML + paginationHTML;
        document.getElementById('rental-schedule-container').innerHTML = fullHTML;
    })
    .catch(error => {
        ...
        document.getElementById('rental-schedule-container').innerHTML = `
            <div class="text-center py-10 bg-red-50 rounded-2xl border border-red-200">
                ...
                <p class="text-red-600 mb-4">Terjadi kesalahan saat memuat jadwal penyewaan.</p>
                <button onclick="location.reload()" class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg transition">
                    Muat Ulang Halaman
                </button>
            </div>
        `;
...
function getStatusBadge(status) {
    switch(status) {
        case 'pending':
            return `<div class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-yellow-100 text-yellow-800 animate-pulse">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" viewBox="0 0 20 20" fill="currentColor">
                    <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clip-rule="evenodd" />
                </svg>
                Menunggu
        ...
        case 'active':
            return `<div class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-green-100 text-green-800">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" viewBox="0 0 20 20" fill="currentColor">
                    <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" />
                </svg>
                Aktif
        ...
        case 'completed':
            return `<div class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" viewBox="0 0 20 20" fill="currentColor">
                    <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" />
                </svg>
                Selesai
        ...
        default:
            return `<div class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-red-100 text-red-800">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" viewBox="0 0 20 20" fill="currentColor">
                    <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd" />
                </svg>
                Dibatalkan
