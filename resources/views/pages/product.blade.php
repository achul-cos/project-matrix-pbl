        ...
        <div class="py-10">
              ...
              <table class="min-w-full bg-white border border-gray-200 rounded-lg">
                <thead>
                  <tr class="bg-gray-100">
                    <th class="py-3 px-4 border-b text-left">Penyewa</th>
                    <th class="py-3 px-4 border-b text-left">Mulai</th>
                    <th class="py-3 px-4 border-b text-left">Selesai</th>
                  ...
                  @foreach($product->rentals as $rental)
                    <tr class="hover:bg-gray-50">
                      <td class="py-3 px-4 border-b">
                        <span class="font-mono">{{ sensorNama($rental->user->name) }}</span>
                        @if(Auth::check() && (Auth::user()->isAdmin() || Auth::id() == $rental->user_id))
                          <span class="text-xs text-gray-500 ml-1">({{ $rental->user->name }})</span>
                        @endif
                      </td>
                      <td class="py-3 px-4 border-b">{{ \Carbon\Carbon::parse($rental->booked_start)->translatedFormat('d M Y H:i') }}</td>
                      <td class="py-3 px-4 border-b">{{ \Carbon\Carbon::parse($rental->booked_end)->translatedFormat('d M Y H:i') }}</td>
                      <td class="py-3 px-4 border-b">{{ $rental->duration }} jam</td>
                      <td class="py-3 px-4 border-b">
          ...
          @else
            <div class="text-center py-10">
              <img src="{{ asset('img/icon/kazuha_chibi.png') }}" alt="Empty Schedule" class="w-32 h-32 mx-auto mb-4">
              <p class="text-gray-500">Belum ada jadwal penyewaan untuk komputer ini.
                <br>
                Sewa Dong Peliss...
              </p>
            </div>
            ...
            <div class="flex flex-row gap-8">
              <div class="w-1/2">
                <form action="{{ route('rent.computer', $product) }}" class="space-y-4 flex flex-col" method="GET" id="rent-form">
                  <label for="username" class="font-medium text-gray-700 block mb-1">Username:</label>
...
<script>
  document.addEventListener('DOMContentLoaded', function () {
    ...
    const mulaiDari = new Date(sekarang.getTime() - offsetMs);
